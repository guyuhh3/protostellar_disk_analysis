...
nu = freq*1e9       # [Hz]
#wav = (c/nu)*1e4   # [micron] 
dist = 300.         # Region distance [pc]

plt.close('all')

def plotMomentMaps(moments=[0, 1, 2], imgPath=None, savePath=None, vclip=None, convolve=False):
    """ 
    Parameters
    ----------
    ...
    for i in moments:
        ...
        m_image.plotMomentMap(moment=i, nu0=nu, dpc=1, vclip=vclip[i])

        plt.title('Moment ' + str(i) + ' Map')
     
        if imgPath is not None:
            plt.savefig(os.path.join(imgPath, os.path.basename(imgPath) + 'moment_' + str(i) + '.png'))
        if savePath is not None:
            plt.savefig(os.path.join(savePath, os.path.basename(imgPath) + 'moment_' + str(i) + '.png'))
